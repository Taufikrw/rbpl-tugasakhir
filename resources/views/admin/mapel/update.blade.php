            ...
            <ul class="border shadow max-h-96 overflow-y-auto scrollbar-none">
                @foreach ($list as $content)
                    <button id="{{ $content->matkulName }}"
                        ...
                        onclick="setName(this)">{{ $content->matkulName }}</button>
                @endforeach
            </ul>
        </div>
        <form action="{{ route('mata-pelajaran.update', $mapel->id) }}" method="POST" class="basis-2/3 relative pl-10 w-full">
            @method('put')
            @csrf
            <div class="mb-4">
                ...
                @enderror"
                    ...
                    value="{{ old('matkulName', $mapel->matkulName) }}">
                @error('matkulName')
            ...
            <div class="mb-4">
                ...
                <select name="idClass" id="className" class="w-full border py-2 px-4 flex items-center rounded">
                    @foreach ($kelas as $option)
                        <option value="{{ $option->id }}" {{ old('idClass', $mapel->idClass) == $option->id ? 'selected' : '' }}>{{ $option->classLevel }} {{ $option->className }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <select name="idGuru" id="teacherName" class="w-full border py-2 px-4 flex items-center rounded">
                    @foreach ($guru as $option)
                        <option value="{{ $option->id }}" {{ old('idClass', $mapel->idGuru) == $option->id ? 'selected' : '' }}>{{ $option->firstName }} {{ $option->lastName }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <input
                    ...
                    is_validate
                @enderror"
                    type="number" min="2000" max="2099" id="tahunAjaran" name="tahunAjaran"
                    placeholder="Masukkan tahun ajaran" value="{{ old('tahunAjaran', $mapel->tahunAjaran) }}">
                @error('tahunAjaran')
                    <div class="text-sm text-pink-700 ml-1">{{ $message }}</div>
                @enderror
            </div>
